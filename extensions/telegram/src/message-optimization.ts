...
export function getOptimizedTelegramPayload(payload: ReplyPayload): ReplyPayload {
    ...
    if (!payload.text || payload.text.length <= MAX_TELEGRAM_PREVIEW) {
        return payload;
    }

    // High-density summary for very long messages
    ...
    return {
        ...
        channelData: {
            ...
            telegram: {
                ...
                buttons: [
                    ...(payload.channelData?.telegram?.buttons || []),
                    [{ text: "📄 View Full Transcript", callback_data: "view_full_msg" }]
                ]
            }
        }
    };
